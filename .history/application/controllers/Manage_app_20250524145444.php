...
class Manage_app extends CI_Controller{
	...
	// TRY FUNGSI ABSEN BARU
	public function absen(){
		if ($this->session->userdata('level')!=99 && $this->session->userdata('level')!=88 && $this->session->userdata('level')!=77 && $this->session->userdata('level')!=66) redirect('logout'); 	    
	
		if ($_POST['submitabsen']){
	
			$jamSekarang = $_POST['jam'];
			$tanggalSekarang = $_POST['date'];
	
			$today = $tanggalSekarang;
			$yesterday = date('Y-m-d', strtotime($today . ' -1 day'));
	
			// Cek apakah sudah ada absen hari ini
			$getabsen_today = $this->M_database->cari_absen($today, $_SESSION['id']);
			$getabsen_yesterday = $this->M_database->cari_absen($yesterday, $_SESSION['id']);
	
			// Insert ke data_absen terlebih dahulu
			$insert = array(	
				'ID_USER' => $_SESSION['id'],
				'JAM' => $jamSekarang,
			...
			// Jika absen hari ini sudah ada, berarti ini absen keluar hari ini
			if ($getabsen_today) {
				$querylog = $this->M_database->cari_log($today, $_SESSION['id']);
	
				$interval = strtotime($_SESSION['JAM_KELUAR']) - strtotime($jamSekarang);
				$selisih = $interval > 0 ? floor($interval / 3600).":".floor(($interval % 3600) / 60).":".($interval % 60) : "";
	
				$jammasuk = (date('l', strtotime($today)) == "Friday") ? $_SESSION['JAM_MASUK_JUMAT'] : $_SESSION['JAM_MASUK'];
				$jampulang = (date('l', strtotime($today)) == "Friday") ? $_SESSION['JAM_KELUAR_JUMAT'] : $_SESSION['JAM_KELUAR'];
	
				$update1 = array(	
					'TANGGAL' => $today,
					'JAM_MASUK' => $jammasuk,
					'JAM_KELUAR' => $jampulang,
					'ABSEN_KELUAR' => $tanggalSekarang . " " . $jamSekarang,	
					'KETERANGAN_KELUAR' => $_POST['ket'],
					'ID_KELUAR' => $idlast[0]['ID'],	
					'PULANG_CEPAT' => $selisih,
				);
				$this->M_database->update($update1, $querylog[0]['ID'], "data_log");
	
			// Jika tidak ada absen hari ini, tapi ada kemarin → kemungkinan shift malam, update log kemarin
			} elseif ($getabsen_yesterday) {
				$querylog = $this->M_database->cari_log($yesterday, $_SESSION['id']);
	
				$interval = strtotime($_SESSION['JAM_KELUAR']) - strtotime($jamSekarang);
				$selisih = $interval > 0 ? floor($interval / 3600).":".floor(($interval % 3600) / 60).":".($interval % 60) : "";
	
				$jammasuk = (date('l', strtotime($yesterday)) == "Friday") ? $_SESSION['JAM_MASUK_JUMAT'] : $_SESSION['JAM_MASUK'];
				$jampulang = (date('l', strtotime($yesterday)) == "Friday") ? $_SESSION['JAM_KELUAR_JUMAT'] : $_SESSION['JAM_KELUAR'];
			...
			// Kalau belum ada absen sama sekali (masuk pertama kali)
			} else {
				$interval = strtotime($jamSekarang) - strtotime($_SESSION['JAM_MASUK']);
				$selisih = $interval > 0 ? floor($interval / 3600).":".floor(($interval % 3600) / 60).":".($interval % 60) : "";
				...
				$insert1 = array(	
					...
					'JAM_MASUK' => $jammasuk,
					'JAM_KELUAR' => $jampulang,
					'ABSEN_MASUK' => $tanggalSekarang . " " . $jamSekarang,	
					'KETERANGAN_MASUK' => $_POST['ket'],
					'ID_MASUK' => $idlast[0]['ID'],	
					'TERLAMBAT' => $selisih,	
				);
				$this->M_database->insert("data_log", $insert1);  
			}
